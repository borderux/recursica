export function toFontWeight(value: string) {
  ...
  if (extraLightFontWeight.some((weight) => lowerValue.includes(weight)))
    return 200;
  if (lightFontWeight.some((weight) => lowerValue.includes(weight))) return 300;
  if (regularFontWeight.some((weight) => lowerValue.includes(weight)))
    return 400;
  if (mediumFontWeight.some((weight) => lowerValue.includes(weight)))
    return 500;
  if (semiboldFontWeight.some((weight) => lowerValue.includes(weight)))
    return 600;
  if (boldFontWeight.some((weight) => lowerValue.includes(weight))) return 700;
  if (extraBoldFontWeight.some((weight) => lowerValue.includes(weight)))
    return 800;
