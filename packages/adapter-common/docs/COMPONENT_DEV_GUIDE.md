...
- **Brand and tokens layers are off-limits without a declared exemption** – Component CSS should only reference `--recursica_ui-kit_components_*`. `--recursica_tokens_*` primitives must never be referenced directly, no exceptions. `--recursica_brand_*` (e.g. the global hover/focus/disabled state tokens above) may only be referenced when the `.module.css` file's own header declares it, one variable per line: `/* recursica-allow-brand: --recursica_brand_states_hover_color */`. `@recursica/token-analyzer` enforces both rules and fails the build on any unexempted usage — see that package's README for the full directive syntax.
