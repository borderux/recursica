...
# Component Storybook Guide (Canonical)
...
- **Example:** For `src/components/Button/Button.tsx`, add `src/components/Button/Button.stories.tsx`.
...
- **Result:** One interactive story where users can change props and see the result immediately. If your adapter's underlying UI library leaks internal system props into Controls, see your adapter's own delta doc for how to filter them.
...
- **Result:** A set of static stories that can be used for visual regression and that document key states without overwhelming the sidebar.
...
- **Default export:** Meta object with `component`, `title` (e.g. under `UI-Kit/Button`), `parameters.docs.description.component` exclusively documenting the purpose and layout usage of the element explicitly for the AutoDocs layout, and optionally `tags` for docs or tests (e.g., `["autodocs"]`).
...
- **Solution:** Rely strictly on Recursica UI components that you have constructed natively (for example, import our wrapped `<Button>` over the raw library `<Button>`). Use standard library layout primitives for all structural alignments to accurately reflect the design system.
