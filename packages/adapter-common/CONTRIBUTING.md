...
Because these docs are shared, a change here affects `recursica-adapter-mantine-v8`, `recursica-adapter-mui-v7`, and any future adapter simultaneously — each is its own separate repo now, not a package in this monorepo. Before editing:

- Confirm the rule you're changing is genuinely universal, not specific to how one adapter currently happens to work.
- After editing, check whether either adapter repo's delta doc (`docs/` in `recursica-adapter-mantine-v8`/`recursica-adapter-mui-v7`) references the section you changed and needs a corresponding update.
