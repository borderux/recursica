...
This document serves as a guideline for any AI agent interacting with the visual regression tester. It defines how we test, what constitutes a failure, and how to handle acceptable differences between an adapter and `@recursica/adapter-mantine-v8`, the source of truth.
...
- `--update-golden` also prunes: any `test/golden/<story-id>.png` + manifest entry whose story id no longer exists in Storybook (renamed or deleted) is deleted automatically, across the whole project regardless of `--grep` scoping. That's expected cleanup, not data loss — if you didn't mean for a story's golden to disappear, its id is still live in Storybook and something else is wrong.
...
- `manifest.json.lock` (git-ignored) is a transient lock file guarding concurrent workers' writes to `manifest.json` — it's created and deleted within a single run. If a crashed run leaves it behind, delete it before the next run; don't treat it as baseline state.
