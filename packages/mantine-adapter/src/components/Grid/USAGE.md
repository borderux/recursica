...
The `Grid` component maps directly to Mantine's `Grid`/`Grid.Col`. The one deviation from Mantine's native API: the prop for spacing between columns is named `gap` (not Mantine's `gutter`), for consistency with `Flex`, `Stack`, and `Group`. Like Mantine's `gutter`, `gap` accepts either a single value or a per-breakpoint object (e.g. `gap={{ base: "rec-sm", md: "rec-xl" }}`), with `rec-*` tokens resolved inside the object. Everything else — `columns`, `grow`, `justify`, `align` on `Grid`, and `span`, `offset`, `order`, `visibleFrom`, `hiddenFrom` on `Grid.Col` — matches Mantine's own naming and accepts the same shapes, including per-breakpoint objects (`{ base, xs, sm, md, lg, xl }`) where Mantine supports them.
