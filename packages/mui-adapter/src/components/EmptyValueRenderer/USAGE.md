...
This document describes how to integrate and use the `EmptyValueRenderer` component in your projects using `@recursica/mui-adapter`.
...
```tsx
import { EmptyValueRenderer } from "@recursica/mui-adapter";
```
...
```tsx
import { EmptyValueRenderer, ReadOnlyField } from "@recursica/mui-adapter";
