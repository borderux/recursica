...
export const recommend_component: Command = {
  name: "recommend_component",
  description:
    "Recommend the best Recursica components to use for a given UI design requirement or functional layout scenario.",
  inputSchema: {
    type: "object",
    properties: {
      requirement: {
  ...
  handler: async (args, { allAdapters }) => {
    ...
    if (matchedRecommendation) {
      ...
      output += `*Tip: To see detailed API properties, call tool \`get_component_doc\` for component name "${matchedRecommendation.component}".*\n`;
    } else {
      ...
      output += `1. **Check the Full List**: Use tool \`list_components\` to check all available UI components in your active adapters.\n`;
