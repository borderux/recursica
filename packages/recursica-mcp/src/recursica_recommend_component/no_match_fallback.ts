...
3. **Pausing Integration / Contributing**: According to Recursica guidelines, if the adapter does not yet implement a required component, the best approach is to pause integration, open the adapter's own repo (e.g. \`@recursica/adapter-mantine-v8\`), and contribute the missing wrapper component following its \`CONTRIBUTING.md\` guidelines.\n`;
