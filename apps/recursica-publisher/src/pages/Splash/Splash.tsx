...
export default function Splash() {
  ...
  // Wait a bit for auth to load, then check authentication status
  useEffect(() => {
    // Give auth provider time to load auth data
  ...
  useEffect(() => {
    ...
    console.log("[Splash] Component mounted", { isAuthenticated, accessToken });
    ...
    // After fade-out completes (~500ms), navigate based on auth status
    const navigateTimer = setTimeout(() => {
      if (isAuthenticated && accessToken) {
        console.log("[Splash] Navigating to home (authenticated)");
        navigate("/");
      } else {
        console.log("[Splash] Navigating to auth (not authenticated)");
        navigate("/auth");
      }
    }, 3500);
  ...
  }, [navigate, isAuthenticated, accessToken, authChecked]);

  return (
    <div
      className={`${classes.splashContent} ${classes.splashContainer} ${
        isFadingOut ? classes.fadeOut : ""
      }`}
    >
      <svg
        width="128"
      ...
      <h1 className={classes.title}>Recursica</h1>
    </div>
  );
}
